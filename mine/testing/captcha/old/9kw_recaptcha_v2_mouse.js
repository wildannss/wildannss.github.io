...
//
// The script fill the captcha of recaptcha v2 demo (mouse).
//
...
function recaptchav2_9kw(){
	...
	for(var i3=1;i3<=30;i3++){
		...
		if(frameDoc2.getElementsByClassName('recaptcha-checkbox-checked').length > 0){
			...
			var selection_element = "";
			try{
				selection_element = document.querySelectorAll('iframe[title*="recaptcha challenge"]')[0].contentDocument.body.getElementsByTagName('div')[1].getElementsByTagName('div')[0].getElementsByTagName('div')[1];
			}catch(err){
			}
			if(!selection_element){
				selection_element = document.querySelectorAll('iframe[title*="recaptcha challenge"]')[0].contentDocument.body.getElementsByTagName('div')[1].getElementsByTagName('div')[1];
			}
			var selection_element_main = document.querySelectorAll('iframe[title*="recaptcha challenge"]')[0].parentNode;
			var selection;
			
			var de = document.documentElement;
			var box = selection_element.getBoundingClientRect();
			var box_main = selection_element_main.getBoundingClientRect();
			var new_top = box_main.top + window.pageYOffset - de.clientTop;
			var new_left = box_main.left + window.pageXOffset - de.clientLeft;
			...
			var new_width = parseInt(parseInt(selection.width)/2);
			var new_height = parseInt(parseInt(selection.height)/2);
			...
			if(document.querySelectorAll('iframe[title*="recaptcha challenge"]')[0].parentNode.parentNode.style.visibility != 'hidden' && document.querySelectorAll('iframe[title*="recaptcha challenge"]')[0].parentNode.parentNode.style.opacity != 0){
				...
				if(boxes.length > 1 || click_captcha == 1){
					imacros_main2 += "TAG POS=1 TYPE=INPUT:CHECKBOX FORM=ACTION:/index.cgi ATTR=NAME:multimouse CONTENT=YES\n";
					imacros_main2 += "TAG POS=1 TYPE=INPUT ATTR=NAME:oldsource CONTENT=recaptchav2\n";
					imacros_main2 += "TAG POS=1 TYPE=INPUT ATTR=NAME:numeric CONTENT=0\n";
				}else{
					...
					if(boxes.length > 1 || click_captcha == 1){
						...
						if(myarray.length < 1){
							...
							for(var i = 0;i < myarray.length; i++){
								var myarray2 = myarray[i].split('x');
								myarray2[0] = parseInt(parseInt(myarray2[0])*2);
								myarray2[1] = parseInt(parseInt(myarray2[1])*2);
								utils.sendMouseEvent("mousedown", parseInt(myarray2[0])+selection.left, parseInt(myarray2[1])+selection.top, 0, 1, 0);
								utils.sendMouseEvent("mousemove", parseInt(myarray2[0])+selection.left, parseInt(myarray2[1])+selection.top, 0, 0, 0);
								utils.sendMouseEvent("mouseup", parseInt(myarray2[0])+selection.left, parseInt(myarray2[1])+selection.top, 0, 1, 0);
							}
						}
					}else{//text captcha
...
function absolutePosition(el) {
    ...
    if(!offsetBase && window.document.body){
        offsetBase = absolutePosition.offsetBase = window.document.createElement('div');
        offsetBase.style.cssText = 'position:absolute;left:0;top:0';
        window.document.body.appendChild(offsetBase);
    }
    if(el && el.ownerDocument === window.document && 'getBoundingClientRect' in el && offsetBase){
